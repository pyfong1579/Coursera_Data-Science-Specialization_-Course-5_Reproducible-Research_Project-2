...
p1f <- p1f + labs(title= 'Fatalities by Event Type', y=NULL, x= 'Count')
...
p2i <- p2i + labs(title= 'Injured by Event Type', y= NULL, x= 'Count')
...
p3p <- p3p + labs(title= 'Property Damage Values by Event Type', y= NULL, x= 'Value ($B)')
...
p4c <- p4c + labs(title= 'Crop Damage Values by Event Type', y= NULL, x= 'Value ($B)')
